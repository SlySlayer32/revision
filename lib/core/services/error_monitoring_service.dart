...
class ErrorMonitoringService {
  ...
  void reportSuccess(String operation, Duration responseTime, {
    Map<String, dynamic>? context,
  }) {
  ...
  Map<String, dynamic> getDiagnostics() {
    ...
    return {
      'systemHealth': {
        ...
        'criticalErrors24h': recentErrors.where((e) => _isCriticalError(e.error)).length,
      },
      'errorCounts': Map<String, dynamic>.from(_errorCounts),
      'lastErrorTimes': _lastErrorTimes.map((k, v) => MapEntry(k, v.toIso8601String())),
      'averageResponseTimes': _averageResponseTimes.map((k, v) => MapEntry(k, '${v.inMilliseconds}ms')),
  ...
  Map<String, dynamic> getErrorTrends() {
    ...
    final errors1h = _errorHistory.where((e) => e.timestamp.isAfter(last1h)).length;
  ...
  void clearOldErrors({Duration? olderThan}) {
    ...
    _errorCounts.removeWhere((operation, _) => !activeOperations.contains(operation));
    _lastErrorTimes.removeWhere((operation, _) => !activeOperations.contains(operation));
  ...
  void _updateHealthStatus() {
    ...
    _isHealthy = recentErrors < 5; // Unhealthy if more than 5 errors in 5 minutes
...
extension ErrorMonitoringIntegration on AIErrorHandler {
  void reportToMonitoring(String operation, dynamic error, {
    StackTrace? stackTrace,
    Duration? responseTime,
    Map<String, dynamic>? context,
  }) {
  ...
  void reportSuccessToMonitoring(String operation, Duration responseTime, {
    Map<String, dynamic>? context,
  }) {
