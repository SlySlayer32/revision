...
void main() {
  group('Gemini 2.5 Flash First Request Tests', () {
    ...
    test('should initialize Firebase AI and create Gemini 2.5 Flash model', () async {
      try {
    ...
    test('should send first request to Gemini 2.5 Flash - Simple Text Generation', () async {
      try {
        // Initialize the Gemini Developer API backend service
        final model = FirebaseAI.googleAI().generativeModel(model: 'gemini-2.5-flash');
        
        // Provide a prompt that contains text - Simple test prompt
        final prompt = [Content.text('Hello Gemini! Please respond with "Hello from Firebase AI!"')];
    ...
    test('should send creative request to Gemini 2.5 Flash - Story Generation', () async {
      try {
        // Initialize the Gemini Developer API backend service
        final model = FirebaseAI.googleAI().generativeModel(model: 'gemini-2.5-flash');
        
        // Provide a creative prompt (following the Firebase AI documentation example)
        final prompt = [Content.text('Write a short story about a magic backpack that helps a photographer edit images.')];
    ...
    test('should test image editing specific prompt with Gemini 2.5 Flash', () async {
      try {
        // Initialize the Gemini Developer API backend service
        final model = FirebaseAI.googleAI().generativeModel(model: 'gemini-2.5-flash');
